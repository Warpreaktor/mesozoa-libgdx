...
public final class GameSimulation {
    ...
    private void dinosaurPhase() {
        for (Dinosaur dinosaur : new ArrayList<>(dinosaurs)) {
            if (dinosaur.captured || dinosaur.removed || dinosaur.trapped) continue;
    ...
    private void checkTrapCapture(Dinosaur dinosaur) {
        ...
        if (dinosaur.trapped || dinosaur.captured || dinosaur.removed) return;

        for (PlayerState player : players) {
            for (Trap trap : player.traps) {
                if (trap.active && !trap.hasDinosaur() && trap.position.equals(dinosaur.position)) {
                    trap.trappedDinosaurId = dinosaur.id;
                    dinosaur.trapped = true;
                    dinosaur.trappedByPlayerId = player.id;
                    log("ЛОВУШКА: " + dinosaur.species.displayName + " #" + dinosaur.id
                            + " попался в ловушку игрока " + player.id + " на " + trap.position);
                    return;
                }
            }
        }
