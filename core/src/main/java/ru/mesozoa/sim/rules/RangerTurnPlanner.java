...
public final class RangerTurnPlanner {
    ...
    private boolean hasUsefulEngineerAction(PlayerState player) {
        boolean hasTrapWithWrongDinosaur = player.traps.stream()
                .filter(trap -> trap.active && trap.hasDinosaur())
                .anyMatch(trap -> simulation.dinosaurs.stream()
                        .filter(dinosaur -> dinosaur.id == trap.trappedDinosaurId)
                        .anyMatch(dinosaur -> !player.needs(dinosaur.species)));

        if (hasTrapWithWrongDinosaur) {
            return true;
        }

        boolean hasVisibleTrapTarget = simulation.dinosaurs.stream()
                .filter(d -> !d.captured && !d.removed && !d.trapped)
                .filter(d -> player.needs(d.species))
                .anyMatch(d -> d.species.captureMethod == CaptureMethod.TRAP);

        if (!hasVisibleTrapTarget) {
            return false;
        }

        long activeTraps = player.traps.stream().filter(t -> t.active).count();
        if (activeTraps < simulation.inventoryConfig.maxTrapsPerPlayer) {
            return true;
        }

        return player.traps.stream().anyMatch(trap -> trap.active && !trap.hasDinosaur());
    }
    ...
    private boolean hasUsefulDriverAction(PlayerState player) {
        boolean hasTrappedDinosaurToExtract = simulation.dinosaurs.stream()
                .anyMatch(dinosaur -> dinosaur.trapped
                        && !dinosaur.captured
                        && !dinosaur.removed
                        && dinosaur.trappedByPlayerId == player.id);

        return hasTrappedDinosaurToExtract
