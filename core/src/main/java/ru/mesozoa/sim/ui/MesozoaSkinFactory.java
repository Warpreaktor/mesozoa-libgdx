 ...
 *
 * Внешние скины из gdx-skins можно подключить позже, когда появится выбранный
 * JSON/atlas. Сейчас фабрика даёт тот же принцип работы через Skin, но без
 * зависимости от дополнительных файлов ассетов. Люди, конечно, любят начинать
 * с atlas'ов и потом неделю искать потерянную ninepatch-рамку, но мы попробуем
 * жить чуть менее больно.
 */
